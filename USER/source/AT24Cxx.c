...
/*
 ...
 7.check sum            1byte  0x0e   
 Ver02
 1.Start frame          1byte  0x00  //00 address set 0xaa 
 2.Device Code          1byte  0x01    
 3.curent ID            4bytes 0x02-0x05 
 //4.Segment ID         2bytes 0x06-0x07 
 5.Year                 1bytes 0x06 
 6.Month                1bytes 0x07
 
 8.Start frame          1byte  0x10  // address set 0xaa  
...
15.check sum            1byte  0x29 

16.Test Mode            1byte  0x2a 
0x01-校准测试 (写入校正参数和分配ID)
0x02-出厂测试 (判断计量是否准确和打印ID)
*/
...
*/
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9|GPIO_Pin_10;  //PD9 ->SCL PD10 ->SDA
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP ;      //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOD, &GPIO_InitStructure);
	GPIO_SetBits(GPIOD,GPIO_Pin_9|GPIO_Pin_10); 	         //PB10,PB11 输出高
...
{				  
	u8 temp=0;		  	    																 
    IIC_Start();  
	if(EE_TYPE>AT24C16)  //大于16kbit（2kByte）的存储器高地址独自站一个字节
	{
		IIC_Send_Byte(0XA0);	   //发送写命令
		IIC_Wait_Ack();
		IIC_Send_Byte(ReadAddr>>8);//发送高地址	 
	}else{//小于16kbit（2kByte）的存储器高地址与A0一起共用一个字节
