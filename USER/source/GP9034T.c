...
//	unsigned char ucCnt;
//	//装载ID
//	Data2String(pcPrint1, PRINT_YEAR_MONTH, ulPhyID, ucDeviceCode);
//	Data2String(pcPrint2, PRINT_YEAR_MONTH, ulPhyID, ucDeviceCode);
//	GP9034T_SetParm();
//	//发送第1联的参数
//	GP9034T_SendString(pucPrint1Location);
//	GP9034T_SendString(pcPrint1);
...
void GP9034T_PrintData(unsigned long ulPhyID,unsigned char ucDeviceCode,unsigned char ucYear,unsigned char ucMonth)
{
	...
	unsigned char ucLen;
	unsigned char ucCnt;
	unsigned short int uiYearMonth = 0;
	uiYearMonth = ucYear;
	uiYearMonth = (uiYearMonth*100) + ucMonth;
	//装载ID
	Data2String(pcPrint1, uiYearMonth, ulPhyID, ucDeviceCode);
	Data2String(pcPrint2, uiYearMonth, ulPhyID, ucDeviceCode);
	GP9034T_SetParm();
	//发送第1联的参数
	GP9034T_SendString(pucPrint1Location);
	GP9034T_SendString(pcPrint1);
